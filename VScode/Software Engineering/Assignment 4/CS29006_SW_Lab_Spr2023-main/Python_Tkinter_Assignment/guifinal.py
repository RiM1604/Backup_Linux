...
def process(clicked,captioner, classifier):
    ...
    if filepath== None:
        ...
        if clicked=="Image Classification":   
            classlist= captioner.__call__(filepath) 
            classes= "Here are the classifications :"
            for i in range(len(classlist)):
                classes= classes+" "+str(i+1)+"."+ str(classlist[i]) + '\n'

            process_label = Label(root, text= classes)
            process_label.grid(row=2+i, column= 1, columnspan=3)
...
if __name__ == '__main__':
    ...
    process_button = Button(root, text="Process", command= lambda: process(choice.get(),captioner,classifier))
